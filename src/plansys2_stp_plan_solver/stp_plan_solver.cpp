  ...
  const std::string & plugin_name)
{
  ...
  if (planner_path == NULL) {
    RCLCPP_FATAL(
      lc_node_->get_logger(), "'STP_HOME' environment variable not defined for %s",
      plugin_name.c_str());
    exit(-1);
  }
  ...
  const std::string & node_namespace)
{
  ...
  status = system(
    (stp_path_ + "/preprocess/preprocess < " + output_sas_file_path.string()).c_str()
  );
  if (status == -1) {
  ...
  // Search for a plan using STP
  const auto pddlplan_file_path = output_dir / std::filesystem::path("pddlplan");
  status = system(
    (stp_path_ + "/search/search y Y a T 10 t 5 e r O 1 C 1 p " +
  ...
  const std::string & node_namespace)
{
  ...
  // STP requires the problem to match the domain's problem name.
